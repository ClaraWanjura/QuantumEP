...
def initialize_phase_sensor_parameters(phase_sensor,use_optimizer=None,do_overwrite=True):
  """
  ...
  for training_step in training_range: # training loop
    ...
    if test_samples is not None:
        if training_step%test_step==0:
            ...
            np.savez(f"tmp.npz",
              sys_sens_couplings=theta[:num_sys_sens],
              sens_couplings=theta[num_sys_sens:],
...
def prepare_Kitaev_phase_sensor(Nx,Ny,twist,N_sens):
  N = Nx*Ny+N_sens
  N_sys = Nx*Ny
  paulis = produce_XYZ(N)
  X,Y,Z = paulis
  ...
  for jx in range(0,Nx):
    for jy in range(0,Ny):
      if ((jx+jy) % 2) == 0:
        H_sys_term_x += X[index_2d(jx,jy,Nx)] @ X[index_2d((jx+1)%Nx,jy,Nx)]
        H_sys_term_z += Z[index_2d(jx,jy,Nx)] @ Z[index_2d((jx+twist)%Nx,(jy-1)%Ny,Nx)]
      else:
        H_sys_term_y += Y[index_2d(jx,jy,Nx)] @ Y[index_2d((jx+1)%Nx,jy,Nx)]
        
  H_sys_terms = [H_sys_term_x, H_sys_term_y, H_sys_term_z]

  # prepare Hamiltonian terms for cluster Ising
  H_sys_terms=get_H_sys_terms_for_cluster_Ising(N_sys,paulis)

  # the sensor may couple to these two neighboring spins:
  #sys_operators=[X[0],Y[0],Z[0],X[index_2d(0,1,Nx)],Y[index_2d(0,1,Nx)],Z[index_2d(0,1,Nx)],Z[0]@Z[index_2d(0,1,Nx)],X[0]@X[index_2d(1,0,Nx)],Y[index_2d(1,0,Nx)]@Y[index_2d(2,0,Nx)]]
  #sys_operators=[X[0],Y[0],Z[0],X[index_2d(0,1,Nx)],Y[index_2d(0,1,Nx)],Z[index_2d(0,1,Nx)],Z[0]@Z[index_2d(0,1,Nx)]]
  sys_operators=[Z[0]@Z[index_2d(0,1,Nx)],X[0]@X[index_2d(1,0,Nx)],Y[index_2d(1,0,Nx)]@Y[index_2d(2,0,Nx)]]
  ...
  # the one outpu 'variable' of the sensor:
  ZA=Z[N_sys]

  # output terms will be the PROJECTORS onto Z,Z=+1,+1 and
  # Z,Z=+1,-1 and Z,Z=-1,-1
  output_terms=[ 0.5*(one+ZA),
                 0.5*(one-ZA)
                 ]
...
# define training sampler: uniform sampling within the triangle of each phase
def sample_batch_Kitaev_phase_sensor(batch_size):
  """
  Returns tuple (x_samples, y_samples), for uniformly sampled
  locations x_samples inside the triangle of each phase, and denoting
  each of them with a 2D vector according to the phase it is in.
  Returns one random sample for each phase, i.e., if batch_size = 1, it
  returns 4 samples, one in each phase.
  """

  batch_size_new = batch_size // 6

  for jj in range(batch_size_new):
    # phase I: Jx >= Jy + Jz
    # sample Jx in [0.5,1), Jy in [0,0.5), compute Jz = 1-Jx-Jy
    Jz = -1
    while (Jz < 0):
        Jx = np.random.uniform(low=0.5,high=1.0)
        Jy = np.random.uniform(low=0.0,high=0.5)
        Jz = 1-Jx-Jy
    if (jj == 0):
        samples = np.array([[Jx,Jy,Jz]])
        outputs = np.array([[1,0]])
    else:
        samples = np.append(samples, np.array([[Jx,Jy,Jz]]),axis=0)
        outputs = np.append(outputs, np.array([[1,0]]),axis=0)
    
    # phase II: Jy >= Jz + Jx
    # sample Jy in [0.5,1), Jz in [0,0.5), compute Jx = 1-Jy-Jz
    Jx = -1
    while (Jx < 0):
        Jy = np.random.uniform(low=0.5,high=1.0)
        Jz = np.random.uniform(low=0.0,high=0.5)
        Jx = 1-Jy-Jz
    samples = np.append(samples, np.array([[Jx,Jy,Jz]]),axis=0)
    outputs = np.append(outputs, np.array([[1,0]]),axis=0)

    # phase III: Jz >= Jx + Jy
    # sample Jz in [0.5,1), Jx in [0,0.5), compute Jy = 1-Jz-Jx
    Jy = -1
    while (Jy < 0):
        Jz = np.random.uniform(low=0.5,high=1.0)
        Jx = np.random.uniform(low=0.0,high=0.5)
        Jy = 1-Jz-Jx
    samples = np.append(samples, np.array([[Jx,Jy,Jz]]),axis=0)
    outputs = np.append(outputs, np.array([[1,0]]),axis=0)

    # phase IV: Jx <= Jy + Jz & Jy <= Jz + Jx & Jz <= Jx + Jy
    # sample Jx in [0,0.5), sample Jz in [0,0.5), compute Jy = 1-Jz-Jx
    Jy = 1
    while (Jy > 0.5):
        Jx = np.random.uniform(low=0.0,high=0.5)
        Jz = np.random.uniform(low=0.0,high=0.5)
        Jy = 1-Jz-Jx
    samples = np.append(samples, np.array([[Jx,Jy,Jz]]),axis=0)
    outputs = np.append(outputs, np.array([[0,1]]),axis=0)
